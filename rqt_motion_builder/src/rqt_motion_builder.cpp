#include <rqt_motion_builder/rqt_motion_builder.h>
...
namespace pal
{
const std::string RQTMotionBuilder::GOTO_MENU = "Go To Position";
const std::string RQTMotionBuilder::DELETE_MENU = "Delete";
const std::string RQTMotionBuilder::SET_TO_CURRENT_MENU = "Recapture frame";
const std::string RQTMotionBuilder::COPY_BELOW_MENU = "Copy Below";
const std::string RQTMotionBuilder::COPY_LAST_MENU = "Copy as Last";

RQTMotionBuilder::RQTMotionBuilder()
  : rqt_gui_cpp::Plugin(), widget_(0), editing_(false), updating_list_(false), editing_motion_("")
{
  setObjectName("RQTMotionBuilder");
}

void RQTMotionBuilder::initPlugin(qt_gui_cpp::PluginContext &context)
{
  // create QWidget
  widget_ = new QWidget();
  ...
  builder_client_.reset(new BMAC(getNodeHandle(), "/ros_motion_builder_node/build"));
  run_motion_client_.reset(new RMAC(getNodeHandle(), "/ros_motion_builder_node/run"));
  list_joints_client_ = getNodeHandle().serviceClient<ros_motion_builder_msgs::ListJointGroups>(
      "/ros_motion_builder_node/list_joint_groups");
  edit_motion_client_ = getNodeHandle().serviceClient<ros_motion_builder_msgs::EditMotion>(
      "/ros_motion_builder_node/edit_motion");
  change_joints_client_ = getNodeHandle().serviceClient<ros_motion_builder_msgs::ChangeJoints>(
      "/ros_motion_builder_node/change_joints");
  properties_dialog_->init(getNodeHandle());
}

void RQTMotionBuilder::shutdownPlugin()
{
  if (editing_)
  {
...
void RQTMotionBuilder::onNewPressed()
{
  ...
  {
    ros_motion_builder_msgs::BuildMotionGoal goal;  // Empty goal for new motion
    builder_client_->sendGoal(goal);
    editing_ = true;
    enableBtns();
    ros::Duration(0.1).sleep();  // Wait for a bit

    ros_motion_builder_msgs::ListJointGroups ljg_service;
    if (list_joints_client_.call(ljg_service))
    {
      ...
      ros_motion_builder_msgs::EditMotion em;
      em.request.action = ros_motion_builder_msgs::EditMotion::Request::LIST;
...
void RQTMotionBuilder::loadMotion(const ros_motion_builder_msgs::Motion &motion)
{
  updating_list_ = true;
...
void RQTMotionBuilder::listMotion()
{
  ros_motion_builder_msgs::EditMotion em;
  em.request.action = ros_motion_builder_msgs::EditMotion::Request::LIST;
  if (edit_motion_client_.call(em))
...
void RQTMotionBuilder::enableBtns()
{
...
void RQTMotionBuilder::disableBtns()
{
...
void RQTMotionBuilder::runDone(const actionlib::SimpleClientGoalState &,
                               const ros_motion_builder_msgs::RunMotionResultConstPtr &)
{
  enableBtns();
}

double RQTMotionBuilder::getJointPosition(const std::vector<std::string> &joints,
...
void RQTMotionBuilder::onLoadPressed()
{
...
void RQTMotionBuilder::onCaptureClicked()
{
  ros_motion_builder_msgs::EditMotion em;
  em.request.action = ros_motion_builder_msgs::EditMotion::Request::APPEND;
  if (edit_motion_client_.call(em))
  {
    if (em.response.ok)
    {
...
void RQTMotionBuilder::onPlayClicked()
{
  if (!motion_running_)
  {
    ros_motion_builder_msgs::RunMotionGoal goal;
    goal.run_mode = ros_motion_builder_msgs::RunMotionGoal::RUN_MOTION;
    goal.downshift = ui_.downshift_->value();

    disableBtns();
    motion_running_ = true;
    run_motion_client_->sendGoal(goal, boost::bind(&RQTMotionBuilder::runDone, this, _1,
...
void RQTMotionBuilder::onSaveClicked()
{
...
void RQTMotionBuilder::onContextMenuRequested(const QPoint &point)
{
...
void RQTMotionBuilder::onGotoSelected(int frame)
{
  ros_motion_builder_msgs::RunMotionGoal goal;
  goal.run_mode = ros_motion_builder_msgs::RunMotionGoal::GO_TO_STEP;
  goal.step_id = frame;

  disableBtns();
  motion_running_ = true;
  run_motion_client_->sendGoal(goal, boost::bind(&RQTMotionBuilder::runDone, this, _1, _2));  // Add done callback to unlock
}

void RQTMotionBuilder::onDeleteSelected(int frame)
{
  ros_motion_builder_msgs::EditMotion em;
  em.request.action = ros_motion_builder_msgs::EditMotion::Request::REMOVE;
...
void RQTMotionBuilder::onSetToCurrentSelected(int frame)
{
  ros_motion_builder_msgs::EditMotion em;
  em.request.action = ros_motion_builder_msgs::EditMotion::Request::EDIT;
  em.request.step_id = frame;
  if (edit_motion_client_.call(em))
  {
    if (em.response.ok)
...
void RQTMotionBuilder::onCopyBelowSelected(int frame)
{
  ros_motion_builder_msgs::EditMotion em;
  em.request.action = ros_motion_builder_msgs::EditMotion::Request::COPY_AS_NEXT;
  em.request.step_id = frame;
  if (edit_motion_client_.call(em))
  {
    if (em.response.ok)
...
void RQTMotionBuilder::onCopyLastSelected(int frame)
{
  ros_motion_builder_msgs::EditMotion em;
  em.request.action = ros_motion_builder_msgs::EditMotion::Request::COPY_AS_LAST;
  em.request.step_id = frame;
  if (edit_motion_client_.call(em))
  {
    if (em.response.ok)
...
void RQTMotionBuilder::onGroupToggled(bool state)
{
  if (state && !updating_list_)
  {
    ...
    ros_motion_builder_msgs::ChangeJoints cj;
...
void RQTMotionBuilder::onJointToggled(bool state)
{
  if (!updating_list_)
  {
...
void RQTMotionBuilder::onCellChanged(int row, int col)
{
  if (col == 0 && !updating_list_)
  {
    ROS_DEBUG_STREAM("Changing time");
    ros_motion_builder_msgs::EditMotion em;
    em.request.action = ros_motion_builder_msgs::EditMotion::Request::EDIT_TIME;
...
void RQTMotionBuilder::onMotionStored(QString motion_name)
{
...
PLUGINLIB_EXPORT_CLASS(pal::RQTMotionBuilder, rqt_gui_cpp::Plugin)
