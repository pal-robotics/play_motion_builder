...
#include <ui_rqt_motion_builder.h>
#include <rqt_motion_builder/properties_dialog.h>
...
namespace pal
{
class RQTMotionBuilder : public rqt_gui_cpp::Plugin
{
  Q_OBJECT
public:
  RQTMotionBuilder();
...
private:
  typedef actionlib::SimpleActionClient<ros_motion_builder_msgs::BuildMotionAction> BMAC;
  typedef actionlib::SimpleActionClient<ros_motion_builder_msgs::RunMotionAction> RMAC;
  ...
  void loadMotion(const ros_motion_builder_msgs::Motion& motion);
  ...
  void runDone(const actionlib::SimpleClientGoalState& state,
               const ros_motion_builder_msgs::RunMotionResultConstPtr& result);
