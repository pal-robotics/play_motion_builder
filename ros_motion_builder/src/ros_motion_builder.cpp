...
{
  ros_motion_builder_msgs::BuildMotionGoalConstPtr goal = action_server_.acceptNewGoal();
  if (goal->motion != "")
  {
    ...
    {
      ros_motion_builder_msgs::BuildMotionResult result;
      result.ok = false;
...
bool ROSMotionBuilderNode::editMotionCb(ros_motion_builder_msgs::EditMotion::Request &req,
                                        ros_motion_builder_msgs::EditMotion::Response &res)
{
  if (running_)
  {
    switch (req.action)
    {
      case ros_motion_builder_msgs::EditMotion::Request::LIST:
        // Don't modify the motion
        break;
      case ros_motion_builder_msgs::EditMotion::Request::APPEND:
      case ros_motion_builder_msgs::EditMotion::Request::EDIT:
      {
        sensor_msgs::JointStateConstPtr joint_state =
            ros::topic::waitForMessage<sensor_msgs::JointState>("/joint_states");
        if (req.action == ros_motion_builder_msgs::EditMotion::Request::APPEND)
        {
      ...
      break;
      case ros_motion_builder_msgs::EditMotion::Request::COPY_AS_LAST:
      case ros_motion_builder_msgs::EditMotion::Request::COPY_AS_NEXT:
        motion_->copyFrame(req.step_id,
                           req.action == ros_motion_builder_msgs::EditMotion::Request::COPY_AS_LAST ?
                               -1 :
                               req.step_id + 1);
        break;
      case ros_motion_builder_msgs::EditMotion::Request::REMOVE:
        motion_->removeKeyFrame(req.step_id);
        break;
      case ros_motion_builder_msgs::EditMotion::Request::EDIT_TIME:
...
bool ROSMotionBuilderNode::storeMotionCb(ros_motion_builder_msgs::StoreMotion::Request &req,
...
bool ROSMotionBuilderNode::changeJointsCb(ros_motion_builder_msgs::ChangeJoints::Request &req,
...
bool ROSMotionBuilderNode::listJointGroupsCb(ros_motion_builder_msgs::ListJointGroups::Request &,
...
void ROSMotionBuilderNode::executeRunMotionCb(const ros_motion_builder_msgs::RunMotionGoalConstPtr &goal)
{
  if (!running_)
  {
  ...
  else
  {
    ...
    if (goal->run_mode == ros_motion_builder_msgs::RunMotionGoal::RUN_MOTION)
    {
      // Load motion to rosparam
      motion_->loadYAML(goal->downshift == 0.0 ? 1.0 : goal->downshift);
    }
    else if (goal->run_mode == ros_motion_builder_msgs::RunMotionGoal::GO_TO_STEP)
    {
    ...
    if (play_motion_client_.getState().state_ == actionlib::SimpleClientGoalState::SUCCEEDED)
      ...
      run_motion_server_.setAborted(ros_motion_builder_msgs::RunMotionResult(),
...
void ROSMotionBuilderNode::motionToROSMsg(ros_motion_builder_msgs::Motion &motion)
{
  motion.joints = motion_->getJoints();
  motion.used_group = motion_->getCurrentGroup();
  for (unsigned int i = 0; i < motion_->size(); ++i)
  {
    ros_motion_builder_msgs::Frame f;
